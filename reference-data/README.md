...
### `forbidden-suffix-rationale.csv` — why a suffix is likely withheld (issue #196)

Categorises the **sourced** rationale families behind a subset of the
ever-forbidden union, keyed by suffix: `category`, `epistemics` (always
`sourced` here — see below), and `source` (a citation key resolved to full
prose by the section renderer). **699** of the 1,466 union suffixes are
classified, across three families, all resting on one primary source — Ofcom's
own FOI response (ref 337399, 2016-09-29, in
`archive/foi/wdtk-356636--all-callsigns-plus-forbidden/raw-extract-all-call-sign-list-nan-smith.md`),
which states the withholding rationale in its own words: an international
obligation (Radio Regulations Art 19.46 et seq) not to allow suffixes that
might be confused with internationally accepted signals — naming `SOS` as its
own worked example and the ITU Q-code series (linking ITU-R Recommendation
M.1172) as a further instance — plus an unwritten "conventional practice" of
withholding suffixes that might spell out generally offensive or
bullying-prone English words, which "does change over time, as taste and
social tolerance change".

- **`itu-q-code`** (676 suffixes) — the entire `QAA`–`QZZ` block (every
  combination, not merely the ones a Q-code table assigns a meaning to); the
  wholesale match corroborates Ofcom's own "programmed not to allow these as
  suffixes" wording as a blanket rule.
- **`itu-operational-abbreviation`** (22 suffixes) — a three-letter token
  ITU-R M.1172's Section II ("Miscellaneous Abbreviations and Signals")
  separately defines (`ADS`, `CFM`, `COL`, `DSC`, `ETA`, `KTS`, `MIN`, `MSG`,
  `MSI`, `NIL`, `PBL`, `PSE`, `RCC`, `REF`, `RPT`, `SAR`, `SIG`, `SLT`, `SVC`,
  `SYS`, `TFC`, `TXT`) — every one of which is independently on the
  forbidden-suffix union, a 100% hit rate against a 22-entry list that is not
  plausibly coincidence.
- **`itu-signal-confusion`** (1 suffix, `SOS`) — the letter's own worked
  example of an internationally accepted signal.

**Deliberately absent**: the remaining 767 suffixes. Ofcom's letter confirms
its "conventional practice" bucket exists and describes its general
character, but publishes no per-suffix mapping, and this project has no
citable authority to attribute any specific three-letter string to it —
guessing which residual suffixes are "offensive" would be exactly the
invented rationale the project's epistemics rule forbids. Absence from this
file means "not attributable to a specific citable rule", never a claim about
any individual suffix; the section index's rationale breakdown quotes Ofcom's
words for the residual bucket collectively, not per suffix. Two further
suffixes worth naming for the record and NOT classified here: `TTT` (safety)
and `XXX` (urgency) are well known amateur/maritime procedural signals, but
ITU-R M.1172 — the only primary document held — does not define them, so they
are left unclassified rather than cited to a document that does not say so.

Generated by `src/ci/forbidden-suffix-rationale.ts` from the committed
`forbidden-suffixes.csv` union; `src/ci/forbidden-suffix-rationale.test.ts`
(`data-validity`) asserts the committed file still matches the generator's
output, so it cannot silently drift. Rendered per suffix (a "why is this
suffix withheld?" section on every per-suffix detail page) and as a
category breakdown quoting Ofcom's own wording (the section index's
`#rationale` anchor) by `src/ci/build-forbidden-section.ts`.
