...
- **What this entry is**: a full amateur-callsign register export listing every
  ...
  — a UTF-8 file with a byte-order mark, CRLF line endings, and **five empty
  trailing columns** the export appends to every row (a single collapsed
  unnamed column, carried in the raw and ignored by the converter). It follows
...
- **Anomalies, recorded not repaired**:
  ...
  - **15 Excel-mangled callsigns** — Intermediate `20xxx`/`21xxx` callsigns
    ...
    export; carried verbatim, never repaired to a guessed suffix (the same
    artefact class seen in earlier register exports).
