...
Two rows below are **membership-derived projections**, not attested
statuses: no source recorded the value in a status column. They are
projected from FOI family membership — the available-pool lists
(availability) and the forbidden-suffix lists (forbiddenness) — which
the ledger deliberately models as membership rather than a per-record
status claim, and are shown here so the availability/forbiddenness the
corpus carries is not lost. The small attested `Available` (a status
a register export itself recorded) is kept as its own, separate row.
...
| `Available` — available-pool membership (projected) | 207,783 | 35,210 | — | 9 | ········· | foi |
| `Live` | 105,353 | 103,594 | — | 3 | ········· | foi |
| `Available` | 12,933 | 3,704 | — | 33 | ▆▆▇▁▄▁▇██ | foi, open-data |
| `Forbidden` | 10,862 | 5,431 | — | 2 | ········· | foi |
| `Forbidden` — forbidden-list membership (projected) | 5,860 | 1,466 | — | 4 | ········· | foi |
