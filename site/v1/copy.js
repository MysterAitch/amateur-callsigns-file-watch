...
export const V1_COPY = {
  ...
  // Record-scoped, plain-English, no verdict words.
  anatomyPage: {
...
// Where a caveat's fuller explanation lives (issue #965). The engine's caveat
// vocabulary is authored in src/ci/state-at-t.ts and its glosses ship verbatim in
// the history manifests; this maps a caveat id to the glossary term that explains
// the reading it invokes, so the folded explainer can offer the definition itself
// instead of a tracker reference. Deliberately PARTIAL – only a caveat whose
// concept has a published term appears, and a caveat with none simply renders its
// gloss. Kept OUT of V1_COPY because these are registry keys, not wording, so the
// claims-bar walk stays a walk over reader-facing prose; a parity assertion in
// src/ci/render/v1-sections.test.ts holds the JS and TS copies identical, and a
// drift guard there rejects a key that is not a real caveat id or a value that is
// not a real glossary term.
/** @type {Readonly<Record<string, keyof typeof V1_COPY.glossary>>} */
export const CAVEAT_GLOSSARY_TERMS = {
