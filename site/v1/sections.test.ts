...
import { renderSiteBar } from './shell.js';
import { datedFactChipParts } from './chip.js';
...
describe('v1 home static/JS parity', { tags: ['unit'] }, () => {
  ...
  // The chip-specific static/JS parity test that used to sit here is retired
  // (ADR 0022): the static chip is now GENERATED by the same renderStatic
  // (site/v1/chip.js) the browser site bar appends, so there is no second
  // implementation left to hold in parity. The cross-page single-source test
  // below still guards the committed stamped pages against a stale deploy.
});
