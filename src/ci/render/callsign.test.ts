...
describe('callsignField wrapper', { tags: ['unit'] }, () => {
  ...
  it('CallsignField_WhenValueCarriesInvisibleCharacters_MarksThemVisibly', () => {
    ...
    expect(callsignField('M7​TEE')).toContain('<span class="marker">{U+200B}</span>');
  });
  ...
  it('CallsignField_WhenValueIsPreMarked_HighlightsExistingMarkersWithoutReMarking', () => {
    // A stats.json example already carries {U+XXXX} markers from derivation
    // time: the wrapper highlights those tokens; it never marks the braces.
    expect(callsignField('G6{U+0020}FMU', { oddCharacters: 'pre-marked' }))
      .toBe(`<code class="${CALLSIGN_CLASS}">G6<span class="marker">{U+0020}</span>FMU</code>`);
  });
...
describe('callsignCharMarker (server twin of site/browser-query.js)', { tags: ['unit'] }, () => {
  ...
  it('CallsignCharMarker_UnnamedInvisibles_ReturnCodepointMarkers', () => {
    expect(callsignCharMarker('​')).toBe('{U+200B}');
    expect(callsignCharMarker('')).toBe('{U+0007}');
