...
describe('runNormaliseSweep', () => {
  ...
  it('Sweep_WhenRawCarriesNonDataLines_EnumeratesThemInMetaAndExcludesFromDerivatives', () => {
    // The line-accounting contract (ratified 2026-07-08): blank separators,
    // export footers and no-callsign rows are excluded from
    // normalised/components/stats ONLY via explicit enumeration against the
    // immutable raw.csv - line number, verbatim content, reason - plus the
    // named header line. Damaged-but-companioned rows are DATA and stay.
    const withFurniture = SALESFORCE_RAW
      + ',,Available,Call Sign - Amateur,21/01/2019,21/01/2019\n' // no callsign
      + '20-Apr,,Allocated,Call Sign - Amateur,21/01/2019,21/01/2019\n' // damaged callsign - stays
      + '\n'
      + '"Generated By:  Someone  21/01/2019 09:24",,,,,\n';
    ...
    const reasons = (meta.ignoredLines ?? []).map(l => l.reason);
    expect(reasons).toContain('no callsign');
    expect(reasons).toContain('blank');
    expect(reasons.some(r => r.includes('export furniture'))).toBe(true);
    expect(meta.ignoredLines).toHaveLength(3);

    // Count invariant: raw physical lines = header + data rows + ignored.
    const rawLines = fs.readFileSync(path.join(tmpRoot, 'archive', '2026-01-01', 'raw.csv'), 'utf8').split('\n');
    if (rawLines[rawLines.length - 1] === '') rawLines.pop();
    expect(rawLines.length).toBe(1 + (meta.files['normalised.csv'].recordCount ?? 0) + 3);

    // The damaged-but-companioned callsign is data and stays; the footer
    // never reaches the derivatives.
    const normalised = fs.readFileSync(path.join(tmpRoot, 'archive', '2026-01-01', 'normalised.csv'), 'utf8');
