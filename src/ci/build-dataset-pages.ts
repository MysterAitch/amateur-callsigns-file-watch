...
export function buildReportPages(outputDir: string, baseUrl: string, foiKeys: string[], series: ReadonlySet<string>, flags: ReadonlySet<string>): string[] {
  ...
  const hubBody = [
    '<h1>Reports</h1>',
    '<p>Standing, deterministic views over the whole archive — regenerated by the report sweep and committed, so each is a stable snapshot whose change in a diff is itself a signal. Everything here derives from the same archived data the <a href="../datasets/index.html">datasets</a> publish and the <a href="../explore.html">Explore</a> page queries.</p>',
...
export function buildDatasetPages(outputDir: string, baseUrl: string = DEFAULT_BASE_URL): DatasetPagesSummary {
  ...
  const indexBody = [
    ...
    '</ul>',
    // Consumer caveat (issue #969): the mirror carries every cell verbatim as a
    // transparency commitment and never rewrites the bytes, so the guidance
    // lives with the reader rather than mutating the data. Spreadsheet software
    // can treat a leading = + - or @ as a formula, so a downloaded CSV is opened
    // with care rather than auto-executed.
    '<p class="download-caveat"><strong>Opening these CSVs:</strong> every cell is carried <strong>verbatim</strong>, exactly as published — the mirror never edits the bytes. Some spreadsheet software treats a cell beginning <code>=</code>, <code>+</code>, <code>-</code> or <code>@</code> as a formula rather than plain text; open a downloaded CSV with caution (for example import it as text, or review it in a plain-text viewer first) rather than letting a spreadsheet auto-execute such cells.</p>',
  ...
  const sitemap = [
    ...
    // The event-time surfaces (issue #726) are generated by
    // build-event-time-surfaces.ts, not this builder, so they are listed here
    // alongside the other static top-level pages rather than pushed into pageUrls.
    `<url><loc>${baseUrl}/on-this-day.html</loc></url>`,
    `<url><loc>${baseUrl}/timeline.html</loc></url>`,
