  ...
  return listFoiEntryKeys(foiDir).map((key) => {
    ...
    if (recordOnly) {
      ...
      const enrichedCell: StageCell = (s.hasNormalised && enrichable)
        ? { state: 'partial', detail: 'enriched collectively in the full-history database (reference joins, components, flags) rather than as a per-entry file' }
...
export const CLASS_BLURBS: Readonly<Record<string, string>> = {
  ...
  // The available-pool discontinuation tracks Ofcom's own words in the same
  // FOI letter cited for the forbidden list (ref 337399): "Due to a system
  // change, the assignment of call signs is now done using an algorithm rather
  // than 'grabbing' from a list." The wording here follows that quote rather
  // than the looser "different computer system" paraphrase it once carried.
  'available-pool':
    'Lists of call signs marked available for issue. Ofcom historically drew new call signs from such lists, but '
    + `after a system change (per ${rationaleSourceLink(GLOSSARY_DEPTH_FROM_ROOT)}) they are now generated by an algorithm on demand `
    + 'rather than drawn from a list, so this series is no longer produced and is closed rather than merely stale. '
    + 'Read an “available” marking with care — see '
