...
export function buildPublishedTiers(dataDir: string): Record<string, number> {
  ...
  const unionCsv = renderObservationsCsv(observations);
  fs.writeFileSync(path.join(dataDir, 'foi-observations.csv.gz'), time('gzip:union-csv', () => zlib.gzipSync(unionCsv, { level: GZIP_LEVEL }), unionCsv.length));
