...
// carries the licence-category and parse-derived field folds. It keeps the small
// shared support (ref, claim, FieldFigures, parseCommittedFieldTable), which the
// sibling flags / status-product files duplicate; each split file runs as its own
// heavy-fold matrix job so no single job carries the whole ~9-minute suite.
