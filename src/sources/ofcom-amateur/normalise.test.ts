...
describe('convertRawCsv', () => {
  ...
  it('Convert_WhenRawCarriesNonDataLines_EnumeratesThemAndKeepsDamagedDataRows', () => {
    // The line-accounting contract: blank lines, all-empty rows, footer
    // furniture and no-callsign rows are excluded via explicit enumeration
    // (line, verbatim content, reason); rows with a callsign AND any
    // companion value are DATA however damaged the callsign looks. The
    // count invariant is exact. Deliberately NO special-casing of
    // never-observed artefacts (e.g. mid-file repeated headers): anything
    // data-shaped stays, surfaces via the flag machinery, and forces a
    // reviewed decision if it ever actually occurs.
    const raw = 'Value,Status,Type\n'
      + 'M7RFT,Allocated,Call Sign - Amateur\n'
      + '20-Apr,Allocated,Call Sign - Amateur\n' // Excel-mangled callsign: data, stays
    ...
    expect(result.ignoredLines).toEqual([
      { line: 4, content: ',Reserved,Call Sign - Amateur', reason: 'no callsign' },
      { line: 5, content: '', reason: 'blank' },
      { line: 6, content: ',,', reason: 'all cells empty' },
      { line: 7, content: 'Generated By:  Someone  01/01/2022 09:00,,', reason: 'no companion values (export furniture, not a register assertion)' },
    ]);
    const rows = lines(result.csv);
    expect(rows).toContain('20-Apr,,Allocated,Call Sign - Amateur,,,,');
    expect(rows.some(r => r.includes('Generated By'))).toBe(false);
    // Invariant: 7 physical lines = 1 header + 2 records + 4 ignored.
  });
