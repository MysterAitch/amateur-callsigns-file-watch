...
// Server-side change signals worth recording alongside the page's own "Updated"
// banner: whichever of these the response carried for the fetch that produced
// the COMMITTED table. Also the shape the next sweep's conditional request
// reads back (If-None-Match / If-Modified-Since) — see readCommittedMeta and
// buildConditionalRequestHeaders in fetch-scc.ts.
export interface SccSourceHeaders {
  etag?: string;
  lastModified?: string;
  date?: string;
}
...
export interface SccMeta {
  ...
  rowCount: number;
  statusCounts: Record<string, number>;
  // Present only when the server sent at least one of these on the fetch that
  // produced the committed table (a plain Apache static file usually sends all
  // three; an .shtml page rendered via SSI may send none, in which case the key
  // is omitted entirely rather than recorded as a set of nulls).
  sourceHeaders?: SccSourceHeaders;
}

// v2 adds the optional sourceHeaders (issue #716): server-side change signals
// recorded ahead of need, and the input the next fetch's conditional-request
// refinement reads back. v1 metadata (no sourceHeaders key) remains valid; the
// field is additive only.
export const SCC_META_SCHEMA_VERSION = 2;

// Only the headers actually present survive into the metadata; an all-absent
// input collapses to undefined so a header-less fetch omits the key entirely
// rather than committing an empty object.
function buildSourceHeaders(headers: SccSourceHeaders | undefined): SccSourceHeaders | undefined {
  if (headers === undefined) return undefined;
  const result: SccSourceHeaders = {};
  if (headers.etag !== undefined) result.etag = headers.etag;
  if (headers.lastModified !== undefined) result.lastModified = headers.lastModified;
  if (headers.date !== undefined) result.date = headers.date;
...
export function toMeta(parsed: ParsedSccTable, opts: { fetchedAt: string; headers?: SccSourceHeaders }): SccMeta {
  if (parsed.updated === undefined) {
    throw new Error('cannot build SCC metadata without a parsed "Updated" banner');
  }
  const sourceHeaders = buildSourceHeaders(opts.headers);
  return {
    ...
    ...(sourceHeaders !== undefined ? { sourceHeaders } : {}),
  };
}
