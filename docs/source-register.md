...
| WDTK 1141667 ("Listing of UK Amateur Radio Callsigns", Ofcom ref 01842686) | 2024-07-22 | pending-ingest | **graduation candidate**: initially fetched as a suspected byte-corroboration of `ofcom-2024-07--call-signs--all-callsigns` under the collect-all-copies posture, but reframed on inspection — a distinct, unheld disclosure, not a copy. Issued-only scope (110,623 rows; reserved 9,792 vs the held snapshot's 51,955), Salesforce-style header; fetched as `Annex 1 All callsigns.xlsx` per its 01842686 letter. Moves to the graduation queue as its own register-family entry — converter variant + goldens still outstanding |
