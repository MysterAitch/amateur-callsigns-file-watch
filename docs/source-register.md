...
| Per-prefix live-callsign counts (Daryl Spence "Ama" FOI) | 2018-07-05 | pending-ingest | counts inline in the WDTK response (20: 8612, 21: 1596, G0: 8615 … **G2: 164, G5: 58**); bears on the G2/M2 story; Ofcom-published copy likely `Amateur-radio-licence-statistics-FOI.pdf`. Ingesting this unblocks the one piece of primary-source support that [`docs/hypothesis-register.md`](hypothesis-register.md) H9 records as **not re-runnable** — the count table omits `M2` entirely — so until then H9 rests on the register corpus alone |
