...
- Date: 2026-07-11
- Related: ADR 0001 (PR-gated in-repo processing), ADR 0002 (write controls), ADR 0003 (frameworkless site, vendored `sql.js-httpvfs`), ADR 0005 (canonical callsign forms), ADR 0006 (componentisation — still *proposed*; the cleaning and placeholder-form rules below are lifted from its component logic, so that part of this record's foundation is unratified, and ADR 0022 is the accepted successor for the component mechanism), ADR 0008 (offline-first PWA), ADR 0010 (archive contract), ADR 0011 (two-tier architecture), ADR 0012 (supply-chain posture), ADR 0021 (the open-data lane freeze that completed this migration), ADR 0024 (expands the serialisation clause into the reasoning, evidence and bar), ADR 0025 (constrains how folds of this ledger may be counted as corroboration — outward-contributed claims are excluded so they cannot return as an independent witness); ADR 0014, ADR 0015, ADR 0016, ADR 0017 and ADR 0018 (the fidelity infrastructure built on this model — each already references this record, so the cross-reference is made symmetric here); issues #361 (exploration tracker), #376 (`/data-status`)
