...
The query databases are a **deploy artefact, derived fresh** and never committed — SQLite is not byte-deterministic, so it lives outside the golden-master lane (ADR 0003, ADR 0010). Building them is expensive: the ~1 GB combined database, the per-dataset tiers, and the full compact claim-ledger fold together took **~22 minutes** in the Pages build job (measured: #513's first deploy, 2026-07-14, 12:52→13:14 UTC — the figure this decision was taken against; the intrinsic build cost has since been reduced, see Consequences).
...
*Implementation status (verified 2026-07-29 against `cicd.yaml`): the packaged-hit and
both-miss paths are live; the middle rung — packaged miss, raw hit, reusing CI's raw
layer — is decided but not yet built. The deploy's `build-site-databases` job restores
only the packaged `pages-db-*` cache and on a miss rebuilds from the committed data.
The naive coupling was trialled and rejected, and the correct shape is tracked as #539
(open) — see Consequences.*

Reusing CI's raw layer means the databases are constructed **once** per input closure, and the deploy's own cache holds only the *incremental* compressed artefacts — so it stops duplicating the ~1 GB raw combined database across the CI and deploy caches, which also **relieves the 10 GB cache pressure**.
...
- A **site-only deploy skips the full database build** (restore + assemble only). Measured on #513 (2026-07-14): the build job fell from **~22 min** (cache miss, full build) to **~3 min** (cache hit) — the residual being the always-run site assembly (dataset pages, nav, size stamping), not the databases. Data and builder changes rebuild correctly, but only the affected layers. The absolute figures are evidence for the decision at its date, not current costs: the free layer (#536, 2026-07-14) cut the full-miss build itself by ~44%, and the later #929 campaign (closed out 2026-07-29) took the whole pipeline from ~39 min to 5.2 min for a warm PR run — current figures live on #929, which re-measures monthly. The *shape* of the win (a packaged hit skips the build entirely; ~3 min site-only deploys, restated on #539, 2026-07-15) is what this decision owns.
- The **databases are constructed once** per input closure (in CI's verify layer, reused by deploy) rather than built separately for verify and publish — the intended end state, which depends on the raw-layer reuse still tracked as #539 (see the implementation-status notes in the Decision and below).
...
- **Implementation is incremental.** Shipped: #507 (smoke-test prompt exit), #513 (the deploy packaged-layer cache), #516 (the `cicd.yaml` unification — `ci.yml` + `pages.yml` merged into one file, deploy gated on `main`, verified live), and #517 (keying the build caches on a test-excluding closure hash, so a test-only change no longer rebuilds the corpus). A separate, cache-independent win landed alongside these: the **free layer** (#536, from the #533 profiling) cut the full-miss database build by roughly 44% (≈940–1124 s → ≈523 s), chiefly by dropping the redundant level-9 gzip twin and tuning the SQLite build PRAGMAs — it reduces the intrinsic build cost the caches otherwise pay on a miss, complementing rather than replacing the layering. Remaining: the **raw-layer reuse** — the walk-down's middle step, where the deploy reuses CI's raw `.tier-cache` / `.ledger-cache` and only gzips/chunks it instead of rebuilding the databases. The naive form — coupling the deploy to the verify jobs with `needs:` — was trialled (#535) and measured a net-negative: the coupling makes the common site-only (packaged-hit) deploy wait for the verify jobs (~3× slower) to pay back only on the rare data-change deploy, so it was rejected. The correct shape — a single shared raw-build job feeding both the verify tests and the deploy packaging, with the packaged-cache short-circuit so the hit path never waits — is filed as #539. That, and any finer per-stage key scoping, are the deferred refinements, to land as their own PRs against this ADR. **Still unbuilt as of 2026-07-29** (#539 open; `build-site-databases` restores only the packaged layer) — until it lands, a data-change deploy constructs the databases twice per input closure (once in CI's verify layer, once in the deploy), so the "constructed once" consequence above holds only for the cached common case, not for the first build of a new closure.
