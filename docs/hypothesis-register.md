...
**Caveat:** every hour label inherits the timezone ambiguity that H3 exists to
classify — the "02:00–04:00" label is local-or-UTC±1h until a source's rendering
timezone is pinned. The *shapes* (a dominant nocturnal cluster, an evening
spike, a flat weekday profile) survive the ambiguity; the exact clock labels do
not.
