...
> **Direction of travel — the canonical-record model has been inverted.** [ADR 0013](docs/adr/0013-raw-keyed-claim-ledger.md) (accepted) makes a **raw-keyed claim ledger** the canonical record, from which the normalised CSVs, query databases, reports and Pages site are all *derived folds*. [ADR 0021](docs/adr/0021-frozen-derived-baseline.md) (accepted) completed the open-data lane's strangler migration: the committed `normalised.csv`/`components.csv`/`stats.json` files freeze as an equivalence baseline for every publication up to the freeze, and the ledger projection is the derivation lane for every publication after it. The FOI lane has since made the same crossing: its text sources' lossless claims were promoted into the ledger by the staged #813 arc (closing #455), the parallel oracle mirrors are deleted, and reconstruction coverage is structural — every registered text source rebuilds from the persisted ledger alone.
...
- **Presentation** ([ADR 0003](docs/adr/0003-in-repo-presentation-poc.md)): every push to `main` builds a SQLite database from the archive + reference data and deploys a frameworkless site to [GitHub Pages](https://mysteraitch.github.io/amateur-callsigns-file-watch/). The deploy root now serves a redesigned **v1 surface** ([`site/v1/`](site/v1/), [#921](https://github.com/MysterAitch/amateur-callsigns-file-watch/issues/921)), currently pre-launch — the whole site carries `noindex` until the one-constant launch flip ([#462](https://github.com/MysterAitch/amateur-callsigns-file-watch/issues/462), `SITE_INDEXABLE`): a callsign-lookup home, an instant per-callsign evidence page (its bi-temporal event/assertion dial answered from prefix-sharded static JSON, no database on that path), on-this-day and timeline **history journeys**, a callsign-**anatomy** structure reference, a plain-language **glossary**, an honest 404, and a how-to-get-the-raw-data page. The established **v0 site** ([`site/`](site/)) — callsign lookup with regional-variant resolution, filtered browse, statistics, in-browser Explore/Compare, per-class dataset index pages (clickable class tags), suffix-availability and RSL matrices, per-publication data-quality flags, a fidelity and integrity deep-dive, the standing reports, and a dedicated forbidden-suffix section (an index, one page per forbidden-list disclosure, and one per ever-forbidden suffix with notable-change drill-downs) — is preserved and isolated under [`/v0/`](https://mysteraitch.github.io/amateur-callsigns-file-watch/v0/). Shared page-render helpers keep each surface reading as one product: [`src/ci/site-render.ts`](src/ci/site-render.ts) for v0, the shell and section registries under [`site/v1/`](site/v1/) for v1.
...
site/                               <- v0 GitHub Pages surface (ADR 0003), now served under /v0/: lookup, per-callsign page, statistics, explore, compare, series, datasets (+ per-class pages), forbidden suffixes, fidelity/integrity, reports, glossary, about
  v1/                               <- redesigned root surface (#921, pre-launch/noindex until #462): lookup, per-callsign evidence, history (on-this-day + timeline), anatomy, glossary, raw-data
src/
...
- **Canonical-record model** — accepted and built ([ADR 0013](docs/adr/0013-raw-keyed-claim-ledger.md), [ADR 0021](docs/adr/0021-frozen-derived-baseline.md)): the raw-keyed claim ledger is canonical, with every other artefact (normalised CSV, query databases, reports, pages) a derived fold over it (tracker [#361](https://github.com/MysterAitch/amateur-callsigns-file-watch/issues/361)). The open-data lane's strangler migration completed at the freeze: the snapshot-canonical items below stay committed as an equivalence baseline up to the freeze, and the ledger projection is the derivation lane from there on.
...
- **Presentation** — built in-repo (ADR 0003): the SQLite database + lookup site deploy to GitHub Pages on every push to `main`. The reader-facing surface is mid-redesign — the root now serves the pre-launch v1 surface (#921), with the v0 site preserved under `/v0/` (see the Presentation bullet above).
...
- **Sort key**: the callsign column, resolved by **name** through the shared header-variant registry (`callsignColumnFor`), so an upstream column reorder cannot silently change what `latest-raw-sorted.csv` is sorted by. A raw file carrying no recognised callsign column falls back to the first column and warns — the warning is itself a data-quality signal, since an unrecognised callsign header means the publication's shape is new. See [ADR 0010](docs/adr/0010-archive-contract.md) for why the derivative exists and why the sort must stay stable.
